...
function applycontent(key, templist, words) {
  ...
  if (words === false) {
    ...
    templist.forEach((item, idx) => {
      ...
      updates[`people/data/${number}/order/${key}/${idx + 1}/quantity`] =
        newQuantity;
    });
  }
...
function deletecheck(name, category, type, index) {
  ...
  document.getElementById("applymodal").addEventListener(
    "click",
    function handler() {
      firebase
        .database()
        .ref(`people/data/${number}/menu/${category}/${type}/${index}/`)
        .remove()
        .then(() => {
...
function editmenufin(key, type, field, category) {
  ...
  const updates = {};
  updates[`people/data/${number}/menu/${category}/${type}/${key}/${field}`] =
    newValue;
...
function editstate(event) {
  ...
  updates[`people/data/${number}/state/reason/img`] =
  ...
  firebase
    ...
    .catch((error) => {
      alertbox(
        `오류 발생, 관리자에게 다음 메시지를 전달해주십시오: ${error}`,
        true,
        false,
      );
    });
  ...
  radio.addEventListener("change", (event) => {
    if (event.target.checked) {
      ...
      firebase
        ...
        .then(() => {
          alertbox(
            `상태가 ${event.target.closest("label")?.textContent.trim()}로 변경되었습니다.`,
            true,
            false,
          );
        })
...
firebase
  .database()
  .ref("/people/data/" + number + "/state")
  .once("value")
  .then((snapshot) => {
    if (snapshot.val()) {
      document.getElementById("editstatetxt").value =
        snapshot.val().reason.message;
